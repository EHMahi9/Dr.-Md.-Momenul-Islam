"""
Gate 5.10 — Master Strategy Evaluation and Comparison Engine
Executes all experimental retrieval strategies on the DEV split and records detailed comparison metrics.
"""
...
def run_evaluation_suite():
    ...
    # 2. Contextual Passages (Doc Title + Section Heading)
    contextual_passages = []
    for c in chunks:
    ...
    strategies_to_test = [
        # (Name, sim_matrix_type, use_bm25_rrf, candidate_depth)
    ...
    for strat_name, mat_type, use_rrf, k_depth in strategies_to_test:
        print(f"\n=======================================================")
        print(f"RUNNING: {strat_name} (Mat={mat_type}, BM25={use_rrf}, Depth={k_depth})")
        print(f"=======================================================")
        ...
        for i, q in enumerate(dev_queries):
            ...
            if use_rrf:
                ...
                candidate_order = np.argsort(-rrf_scores)
            else:
                candidate_order = np.argsort(-dense_scores)

            top_k_indices = candidate_order[:k_depth]
            candidate_cids = [chunks[idx]["chunk_id"] for idx in top_k_indices]

            # Cross-Encoder Rerank (using raw clean chunk text)
            ...
            # Failure movement relative to baseline
        ...
        print(f"  Post-Rerank:    R@1={round(r_r1*100,2)}% ({int(r_r1*n)}/40), R@3={round(r_r3*100,2)}%, R@5={round(r_r5*100,2)}% ({int(r_r5*n)}/40), MRR={round(r_mrr,4)}")
    ...
    print(f"\nSaved all strategy results to {out_file}")
